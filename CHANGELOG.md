...
- **HTML cleaning: Nyiakeng Puachue Hmong U+1E14F CIRCLED CA (FETCH_URL)** — `collapse_whitespace()` maps NYIAKENG PUACHUE HMONG CIRCLED CA (`So`) to ASCII space; not Rust whitespace. Extends FEAT-D91 (Wancho comma/full stop + Ideographic Symbols U+16FE2–U+16FE3). Digits U+1E140–U+1E149 (`Nd`) and Lm category signs / syllable lengthener U+1E137–U+1E13D stay unmapped. Tests extend `wancho_comma_full_stop_and_nyiakeng_sentence_punctuation_separate_words`; `nyiakeng_puachue_hmong_digit_nd_and_category_lm_stay_unmapped` in `commands/html_cleaning.rs`. (FEAT-D201.)
