...
- **Docs: OpenClaw §87–§89 re-verification** — All §7 checks re-run; no discrepancies found (`005-openclaw-reviewer`).
- **FEATURE-CODER backlog** — Clippy clean builds, Chrome lean flags, and Redmine time entry creation rows marked done (`006-feature-coder`).
- **Docs 025: Redmine API skill** — POST time entry documented; create context description updated; open task marked done.
- **022 testing note** — Closing reviewer smoke tests 2026-03-20 (Chrome lean flags, configurable idle timeout, Redmine time entry creation; cargo build, debug.log, agents, monitors UP).
