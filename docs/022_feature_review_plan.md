...
### Closing reviewer smoke test 2026-03-28 (`tasks/TESTING-20260322-1920-openclaw-ollama-warmup-before-channels.md` → `TESTED-…`)

- [x] Entry: **`004-closing-reviewer/CLOSING-REVIEWER-PROMPT.md`** — §9 checklist consulted; scope acotado a **`tasks/TESTING-20260322-1920-openclaw-ollama-warmup-before-channels.md`** (archivo ausente → creado; verificación del gate Ollama antes de Discord/scheduler/heartbeat). **`git diff` / `CHANGELOG.md` / `006-feature-coder/FEATURE-CODER.md`:** sin nuevos **FEAT-D\*** introducidos en esta corrida (solo comprobación de orden en `lib.rs` ya presente + tarea + este bloque). No **`pkill -f mac_stats`** (**AGENTS.md**).
- [x] `cargo check` — zero errors.
- [ ] `cargo clippy --all-targets -- -D warnings` — **not clean** (many pre-existing lints across `browser_agent/`, `commands/`, etc.; first failures include `suspicious_patterns.rs` `redundant_static_lifetimes`, `cdp_downloads.rs` `single_char_add_str`, `cdp_trace_archive.rs` `large_enum_variant`, and further items in `browser_agent/mod.rs` / `ollama.rs` / `model_list_cache.rs` — full bar from prior **2026-03-25** blocks **not** matched).
- [x] `cargo test` — **870** tests pass (`mac_stats` lib crate); **1** doc-test ignored.
- [x] `cargo build --release` succeeds (**v0.1.68**).
- [x] Task verification: **`ensure_ollama_agent_ready_at_startup().await`** runs inside **`tauri::async_runtime::block_on`** **before** **`discord::spawn_discord_if_configured`**, **`scheduler::spawn_scheduler_thread`**, **`scheduler::heartbeat::spawn_heartbeat_thread`** (`lib.rs`); structured log **`Ollama startup warmup finished (gate open); spawning Discord…`** (`mac_stats_startup`); **`docs/015_ollama_api.md`** startup-ordering paragraph aligns. Task file renamed **`TESTED-20260322-1920-openclaw-ollama-warmup-before-channels.md`** (not **`CLOSED-`**) because **clippy -D warnings** failed.
