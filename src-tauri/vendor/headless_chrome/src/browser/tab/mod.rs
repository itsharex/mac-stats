...
impl Tab {
    ...
    /// Reset the handler used for `Fetch.authRequired` to Chrome’s default behaviour (`Default` response).
    ///
    /// mac-stats uses this for non-proxy HTTP authentication challenges after a proxy challenge may
    /// have left [`Self::authenticate`]’s `ProvideCredentials` state in place.
    pub fn reset_fetch_auth_challenge_response_to_default(&self) -> Result<&Self> {
        let mut current_auth_handler = self.auth_handler.lock().unwrap();
        *current_auth_handler = AuthChallengeResponse {
            response: Fetch::AuthChallengeResponseResponse::Default,
            username: None,
