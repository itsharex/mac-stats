...
3. **BROWSER_SCREENSHOT** and **BROWSER_SAVE_PDF**: **BROWSER_SCREENSHOT** — **BROWSER_SCREENSHOT: current** (or empty arg) captures the **focused** CDP tab with no navigation; **BROWSER_SCREENSHOT: <url>** navigates that **same focused tab** to `<url>` then captures (same SSRF, redirect-chain, and policy checks as **BROWSER_NAVIGATE** — not a bypass). After **BROWSER_NAVIGATE … new_tab**, the URL screenshot runs in the **new** tab, not `tabs.first()`. **BROWSER_SAVE_PDF** — export the **current** tab only via CDP `Page.printToPDF` under `~/.mac-stats/pdfs/` (BROWSER_SAVE_PDF: current or empty). **Do not** pass a raw URL to **BROWSER_SAVE_PDF**; use **BROWSER_NAVIGATE** first, then optional clicks, then PDF. Oversized PDFs fail with a clear error (suggest scroll/screenshot or raise **browserArtifactMaxBytes** in config). **Print backgrounds** default off; set **`browserPrintPdfBackground`** true in config (or env **MAC_STATS_BROWSER_PRINT_PDF_BACKGROUND**) if needed. **BROWSER_SAVE_PDF** is **not supported** in HTTP fetch fallback mode — the tool returns an explicit error (no PNG substitute). Discord may attach PDFs when allowed; the tool result always includes the local path.
