...
**`git diff --cached` / `git diff` / `CHANGELOG.md` [0.1.75] / `006-feature-coder/FEATURE-CODER.md`:** Tras el primer **`git mv`**, **`git diff --cached --stat`** — rename **CLOSED→TESTING** de esta tarea (**1** archivo); **`git diff --stat`** (working tree tras el primer **`git mv`**) — **8** tareas **`tasks/CLOSED-*.md`** ajenas al gate Ollama (**145** inserciones en el resumen capturado). Tras el ida/vuelta **`git mv`**, **`git diff --cached`** vacío respecto a esta tarea. Sin **FEAT-D\*** nuevos atribuibles solo al gate Ollama.
